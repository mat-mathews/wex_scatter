...
class TestMapCsToProjectsBatch(unittest.TestCase):
    ...
    def test_subdirectory_files_map_to_parent_csproj(self):
        ...
        for cs_file in [self.cs_a1, self.cs_a2, self.cs_a3]:
            self.assertEqual(
                result[str(cs_file)], resolved, f"{cs_file.name} should map to ProjectA.csproj"
            )
    ...
    def test_nonexistent_path(self):
        ...
        if value is not None:
            self.assertTrue(
                value.endswith(".csproj"), f"Expected None or .csproj path, got {value}"
            )
...
class TestMapCsToProjectsParallel(unittest.TestCase):
    ...
    def test_sequential_fallback_disabled_flag(self):
        ...
        result = scatter.map_cs_to_projects_parallel(self.cs_files, disable_multiprocessing=True)
    ...
    def test_parallel_vs_sequential_consistency(self):
        ...
        sequential = scatter.map_cs_to_projects_parallel(
            self.cs_files, disable_multiprocessing=True
        )

        parallel = scatter.map_cs_to_projects_parallel(
            self.cs_files, max_workers=2, cs_analysis_chunk_size=2, disable_multiprocessing=False
        )

        self.assertEqual(
            sequential, parallel, "Parallel and sequential should produce identical mappings"
        )
    ...
    def test_result_completeness(self):
        ...
        for cs_file in self.cs_files:
            self.assertIn(str(cs_file), result, f"Missing mapping for {cs_file.name}")

    def test_empty_input(self):
        """Empty file list returns empty dict."""
        result = scatter.map_cs_to_projects_parallel([], disable_multiprocessing=False)
        self.assertEqual(result, {})
    ...
    def test_all_galaxy_files_map_to_same_project(self):
        ...
        for cs_file in self.cs_files:
            ...
            self.assertEqual(
                mapped, expected_csproj, f"{cs_file.name} should map to GalaxyWorks.Data.csproj"
            )
...
class TestSprocMappingIntegration(unittest.TestCase):
    ...
    def test_sproc_parallel_vs_sequential_identical_results(self):
        ...
        for proj_path in parallel_results:
            ...
            self.assertTrue(
                len(seq_proj) > 0, f"Project {proj_path.name} missing from sequential results"
            )
            sequential_classes = set(sequential_results[seq_proj[0]].keys())
            self.assertEqual(
                parallel_classes, sequential_classes, f"Classes differ for {proj_path.name}"
            )
    ...
    def test_mapped_projects_are_path_objects(self):
        ...
        for key in result.keys():
            self.assertIsInstance(key, Path, f"Project key should be Path, got {type(key)}")
