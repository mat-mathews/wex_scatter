...
class DbDependency:
    ...
    detection_method: (
        str  # "string_literal", "ef_dbset", "ef_dbcontext", "sql_text", "connection_string"
    )
...
class _FileIndex:
    ...
    def enclosing_class(self, offset: int) -> Optional[str]:
        ...
        idx = bisect.bisect_right(self._class_positions, (offset,), key=lambda x: (x[0],)) - 1
        if idx >= 0:
...
def build_db_dependency_matrix(
    ...
    return {name: sorted(projects) for name, projects in sorted(obj_to_projects.items())}
...
def _discover_project_cs_map(
    ...
    csproj_files = [
        p for p in all_csproj if not any(fnmatch.fnmatch(str(p), pat) for pat in exclude_patterns)
    ]
    ...
    cs_files = [
        p for p in all_cs if not any(fnmatch.fnmatch(str(p), pat) for pat in exclude_patterns)
    ]
