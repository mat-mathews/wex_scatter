...
def find_solutions_for_project(
    ...
    logging.debug(
        f"Searching {len(solution_cache)} cached solution files for '{project_filename}'..."
    )
...
def map_batch_jobs_from_config_repo(app_config_path: Path) -> Dict[str, List[str]]:
    ...
    if not app_config_path or not app_config_path.is_dir():
        logging.warning(
            "App config path not provided or not a valid directory. Skipping batch job mapping."
        )
    ...
    if not target_dir.is_dir():
        logging.warning(
            f"Batch job directory not found: {target_dir}. Cannot map specific job names."
        )
        return batch_job_map
    ...
    try:
        ...
        if job_names:
            ...
            logging.info(
                f"Found {len(job_names)} specific batch jobs in app-config repo (e.g., {', '.join(job_names[:3])}...)."
            )
        else:
            logging.warning(
                f"Found batch job directory, but it contains no subdirectories: {target_dir}"
            )
    except OSError as e:
...
def _build_consumer_results(
    ...
    if not final_consumers_data:
        logging.info(
            f"   No consumers found for target '{target_project_name}' triggered by '{triggering_info}'."
        )
        return

    logging.info(
        f"   Found {len(final_consumers_data)} consumer(s) for target '{target_project_name}' triggered by '{triggering_info}'."
    )
    ...
    for consumer_info in final_consumers_data:
        ...
        logging.debug(
            f"   Found {len(solutions_for_consumer_names)} solutions for consumer '{consumer_name_stem}': {solutions_for_consumer_names}"
        )
